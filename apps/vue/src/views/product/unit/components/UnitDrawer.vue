...
<script lang="ts" setup>
  ...
  import { Unit } from '/@/api/product/unit/model';
  import { useUnitFormContext } from '../hooks/useUnitFormContext';
  ...
  const entity = ref<Unit>({} as Unit);
  const framework = ref<string | undefined>('');
  const formElRef = ref<Nullable<TabFormActionType>>(null);

  const { formTitle, getFormSchemas, handleFormSubmit /*,fetchUnitResource*/ } = useUnitFormContext({
    unitModel: entity,
    formElRef: formElRef,
  });
